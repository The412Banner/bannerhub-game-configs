...
NON_GAME = re.compile(
    r'\b(7[-_ ]?zip|4gb[-_ ]?patch|all[-_ ]?in[-_ ]?one[-_ ]?runtimes|directx|vc[-_ ]?redist|vcredist|'
    r'redist|winrar|dotnet|installer|setup|bootstrap|packagedgame|launcher[-_ ]?redirector)\b', re.I)

def naive(n):  # matches the site's prettyName
    return re.sub(r'\s+', ' ', n.replace('_', ' ')).strip()

def norm(n):   # the big wins: apostrophe (_s_ -> 's), collapse __, strip edition suffixes, split camelCase
    s = n.replace('_s_', "'s ")
    s = re.sub(r'_+', ' ', s)
